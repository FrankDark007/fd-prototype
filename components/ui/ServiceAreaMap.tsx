...
import { MapPin, Building2, Star, Phone, Globe } from 'lucide-react';
...
const ServiceAreaMap: React.FC<ServiceAreaMapProps> = ({ locations, onLocationSelect, className = '' }) => {
  // Calculate bounds with some padding to frame the points nicely
  const bounds = useMemo(() => {
    // Filter out locations without coordinates
    const validLocs = locations.filter(l => l.lat && l.lng);
    
    // Include HQ in bounds calculation to ensure it's always visible
  ...
  const getPosition = (lat: number, lng: number) => {
    ...
    // Map latitude to Y (inverted because SVG Y grows downwards, but Latitude grows upwards)
    // We map maxLat to 0% and minLat to 100%
    ...
    <div className={`relative w-full aspect-[4/3] md:aspect-[21/9] bg-[#f8f9fa] rounded-[32px] border border-gray-200 overflow-hidden shadow-inner ${className}`}>
      ...
      <div className="absolute inset-0 opacity-30 pointer-events-none">
         <svg width="100%" height="100%">
            ...
            {/* Abstract River (Potomac-ish shape) */}
            <path 
      ...
      {/* Locations */}
      <div className="absolute inset-0 m-12 md:m-20">
        {locations.map((loc) => {
           ...
           return (
             <button
                ...
                className="absolute group flex flex-col items-center transform -translate-x-1/2 -translate-y-1/2 transition-all duration-300 hover:z-20 focus:outline-none"
                style={{ top: `${y}%`, left: `${x}%` }}
                aria-label={`Select ${loc.title}`}
             >
                {/* Pin Head */}
                <div className="relative mb-2 transition-transform duration-300 group-hover:-translate-y-1 group-hover:scale-110">
                ...
                {/* Label */}
                <div className="px-2.5 py-1 bg-white/95 backdrop-blur-sm rounded-lg shadow-sm border border-gray-200 text-[10px] md:text-xs font-bold text-gray-700 whitespace-nowrap group-hover:text-primary group-hover:border-primary/20 transition-all opacity-90 group-hover:opacity-100">
                   {loc.title}
                </div>
             </button>
           );
        ...
        {/* HQ Marker (Vienna) */}
        <div 
            className="absolute flex flex-col items-center transform -translate-x-1/2 -translate-y-1/2 z-30 group"
        ...
        >
            ...
            {/* Label (Fades out on hover to reduce clutter) */}
